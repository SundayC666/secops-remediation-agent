    ...
    if "mac os x" in ua or "macos" in ua:
        ...
        if match:
            ...
            if major >= 11 or (major == 10 and minor >= 16):
                ...
                # Since macOS 11 Big Sur (2020), browsers report 10_15_7 for compatibility
                # even when running on macOS 12, 13, 14, or 15.
                # We'll assume the user is running the latest macOS (Sequoia 15)
                version = "15"
                normalized = "macOS 15 Sequoia"
                tags.append("macos_15")
            else:
                version = f"10.{minor}"
...
QUICK_OS_OPTIONS = [
    ...
    {"id": "macos_14", "label": "macOS 14 Sonoma", "query": "macOS 14 Sonoma vulnerabilities"},
